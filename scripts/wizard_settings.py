...
try:
    ...
    print(
        "Ошибка: Не установлена библиотека 'questionary'. Запустите: pip"
        " install questionary"
    )
    sys.exit(1)
    ...
    "setting_schemas": {
        "attribute": [
            ...
            },
            {
                ...
                "type": "boolean",
                "label": {
                    "ru": "Настройки публичны",
                    "en": "Settings are public",
                },
                "width": 1,
                "is_system": True,
                "default": True,
...
class Wizard:
    ...
    def build_select_options(self, active_langs: list) -> dict:
        ...
        while True:
            val = check_exit(
                questionary.text("Value (системное значение):").ask()
            )
            if not val.strip():
                continue

            label_primary = check_exit(
                questionary.text(
                    f"Label (на языке {primary_lang}):"
                ).ask()
            )
            ...
            if not check_exit(
                questionary.confirm("Добавить еще опцию?").ask()
            ):
                break
        return options
    ...
    def run(self):
        ...
        print(
            f"{Colors.HEADER}{Colors.BOLD}VMS-NC Box: Configuration"
            f" Settings Wizard{Colors.ENDC}"
        )
        print("Нажмите Enter, чтобы принять значение по умолчанию.\n")

        langs_str = check_exit(
            questionary.text(
        ...
        self.config["languages"] = [
            l.strip() for l in langs_str.split(",") if l.strip()
        ]
        ...
        for ch in channels:
            ...
            is_pub = check_exit(
                questionary.confirm(
                    f"Канал '{ch}': новые элементы публичны по умолчанию?",
                    default=default_pub,
                ).ask()
            )
        ...
        for entity, fields in list(self.config["setting_schemas"].items()):
            msg = f"Сущность '{entity}' (полей: {len(fields)}). Настроить/добавить поля?"
            edit = check_exit(questionary.confirm(msg, default=False).ask())
            if not edit:
                continue
            ...
            for f in fields:
                ...
                k = check_exit(
                    questionary.text(
                        "Ключ (очистите строку, чтобы удалить):",
                        default=f["key"],
                    ).ask()
                )
                if not k.strip():
                ...
                lbl = (
                    f["label"].get(primary_lang, f["key"])
                    if isinstance(f["label"], dict)
                    else str(f["label"])
                )
                new_lbl = check_exit(
                    questionary.text(
                        f"Название ({primary_lang}):", default=lbl
                    ).ask()
                )
                ...
                f["width"] = int(
                    check_exit(
                        questionary.select(
                            "Ширина (1-4):",
                            choices=["1", "2", "3", "4"],
                            default=str(f.get("width", 1)),
                        ).ask()
                    )
                )
                if f["type"] == "select" and "options" not in f:
                    f["options"] = self.build_select_options(
                        self.config["languages"]
                    )
                new_fields.append(f)
            ...
            while True:
                print(
                    f"\n{Colors.GREEN}Добавление нового поля в"
                    f" {entity}{Colors.ENDC}"
                )
                k = check_exit(
                    questionary.text(
                        "Ключ нового поля (пусто для завершения):"
                    ).ask()
                )
                if not k.strip():
                    break
                ...
                lbl = check_exit(
                    questionary.text(f"Название ({primary_lang}):").ask()
                )
                w = int(
                    check_exit(
                        questionary.select(
                ...
                if t == "select":
                    new_f["options"] = self.build_select_options(
                        self.config["languages"]
                    )
    ...
    def _save(self):
        try:
            ...
            print(
                f"\n{Colors.GREEN}Настройки сохранены:"
