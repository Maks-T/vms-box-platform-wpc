          ...
          <div class="flex flex-col gap-3 mt-1 relative ml-2 pl-4 pr-2 sm:pr-4 pb-2 border-l-2 border-gray-100 dark:border-gray-800">
            @foreach($fields as $childField)
              @if(!empty($childField['is_multiple']))
                @include('valerie-wpc::filament.clusters.pipelines.components.fields.field-group', ['field' => $childField, 'depth' => $depth])
              @elseif(empty($childField['child']['id']) && !empty($childField['static_meta']))
                @include('valerie-wpc::filament.clusters.pipelines.components.fields.field-text', ['field' => $childField])
              @else
                @include('valerie-wpc::filament.clusters.pipelines.components.fields.field-single', ['field' => $childField, 'depth' => $depth])
              @endif
            @endforeach
          </div>
        @endif
